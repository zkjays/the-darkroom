...
export async function PUT(req: NextRequest) {
  ...
  if ("goal_id" in body) {
    ...
    // Denormalized cache — non-critical if endorsement_count column doesn't exist yet
    await db.from("daily_goals").update({ endorsement_count: newCount }).eq("id", goal_id);
